...
String DEVICE_NAME = String("WINDSIM"); //{"Group":"General","Name":"DEVICE_NAME","Title":"Device name","DefaultValue":"SimHub Dash","Type":"string","Template":"String DEVICE_NAME = String(\"{0}\");"}
	...
	if (FlowSerialAvailable() > 0) {
		...
		if (header == MESSAGE_HEADER)
		{
			...
			if (opt == 'P') {
				...
				// -------------------------------------------------------
#ifdef PWM_WIND_SIM
        String msg1 = FlowSerialReadStringUntil(';');
        float pwmSPD = msg1.toFloat();
        int PWMOUT = (int) (pwmSPD/PWM_MAXSPD * 255.0);
        if(PWMOUT > 255) PWMOUT = 255;
        if(PWMOUT < PWM_MIN) PWMOUT =0;
        analogWrite(PWM_OUTPIN,PWMOUT);
#endif
